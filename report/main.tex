...
The main question of my project is: can we predict the rectilinear crossing number of a random graph given the number of edges, nodes, the maximum edge length, minimum edge length, and the closed ring that contains the graph? We can hopefully use these features to predict the crossing number of a random graph and then use that prediction to try and create an idea to plot a set of edges to a set of points.  
...
For phase 0, I first made a C file and shell file to make 1,000 different graphs for each of the other 6 phases. This will be the data that we will use throughout the project so, if there is any error in the C file it will effect the outcome of our results.
...
Now, for phase 1, as you can see from the scatter plots, we got a highly correlated linear regression between the edges and nodes with the crossing number, with an $R^2$ of $.999$ on the training data and $R^2$ of $.999$ on the testing data.
...
Similar to phase 1, I got a linear regression for the inner radius squared to the crossing number of the graph, which got an  $R^2$ of $.966$ on the training data and an $R^2$ of $.968$ on the testing data. I was surprised at that because we thought it would be a little lower, looking at the variance when it is lower, but we guess it is still very well correlated.
...
For phase 5, as we can see from the plots, there is not a strong correlation but still some correlation, which is why we got an $R^2$ of $.509$ on the training data and $R^2$ of $.476$ on the testing data, meaning they are correlated but not as much as phase 1 and phase 4.

Now for the finishing phase, we trained three different linear regression models from sklearn: LinearRegression, Ridge, and LASSO both with and without normalizing the values. For LinearRegression, we looked at every non-empty subset of features to find the subset of features that gave the greatest $R^2$ value: for non-normalized values the subset of nodes, edges, outer radius, maximum distance, and inner - outer radius  gave the greatest $R^2$ with $.988$; and for normalized values the subset nodes, edges, inner radius, minimum distance, and ineer - outer radius gave the greatest $R^2$ with $.988$. For Ridge, we tried to find the best $\alpha$ value that gave the maximum $R^2$, which was $\alpha = 10.985$ with $R^2 = .987$ for non-normalized values and $\alpha = 2.024$ with $R^2 = .988$ for normalized values. Now for LASSO, we tried to find the best $\alpha$ value that gave the minimum mean squared error, which was $\alpha = 1.311 \times 10^{4}$ with a mean squared error of $2.248 \times 10^{8}$ for non-normalized values and $\alpha = .001$ with a mean squared error of $.012$ .

For the final part, we tested each of the three trained linear models on the training data for phase 6 and then use the testing data to test the models. we trained both a non-normalized and a normalized verison for each regression but for simplicity we will only state the values on the normalized models. For LinearRegression, we got an $R^2$ of $.988$ and a mean squared error of $.012$ on the training data and an $R^2$ of $.993$ and a mean squared error of $.009$ on the testing data; For Ridge, we got an $R^2$ of $.988$ and a mean squared error of $.012$ on the training data and an $R^2$ of $.993$ and a mean squared error of $.009$ on the testing data; And for LASSO, we got an $R^2$ of $.988$ and a mean squared error of $.012$ on the training data and an $R^2$ of $.992$ and a mean squared error of $.009$ on the testing data.So, all three regressions did well at predicting the rectilinear crossing number that I will state all three. Let n = nodes, e = edges, r = inner radius, R = outer radius, m = minimum distance, and M = maximum distance; then

\[\text{LinearRegression: } Cr(G) = .009n^4 + .987e^2 + .041r^2 + .006m + .004(R^2 - r^2)\],
\[\text{Ridge: } Cr(G) = .011n^4 + 0.983e^2 + .04r^2 + .005R^2 + .004m - .006M - .021(R^2 - r^2)\], and
\[\text{LASSO: } Cr(G) = .007n^4 + .986e^2 + .038r^2 + .002m - .02(R^2 - r^2)\].
...
Overall, this project was a lot of fun but stressful, as basically, the answer was clear based on the research done on this subject, but I had some ideas I wanted to try and find an answer to those ideas.
\section*{Appendices}
