...
\subsection*{Appendix D - Why z-score fails}
Earlier in my paper, I mentioned that I normalized my data by dividing by the maximum instead of using the z-score, and I will explain that here. If you look through my code, you will see that whenever I change a feature, I will mostly square it because that is what the data provides I do. So, if I first wanted to normalize my data and then look at the graphs, I would need a normalization metric that, when I multiply it, is the same as multiplying that data point, and for a z-score that is not the case. The reason why is that it subtracts the mean, so $z^2$ is the value minus the mean square which is not equal to the squared value minus the squared mean, and thus $z^2$ is not the value squared.  
